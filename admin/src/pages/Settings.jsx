...
// Use relative route because axios baseURL is already http://localhost:5001/api
...
const Settings = () => {
  ...
  // Sync profile details with user state
  useEffect(() => {
    if (user) {
      setName(user.name || "");
  ...
  const saveStoreSettings = async () => {
    try {
      ...
      showError(err.response?.data?.message || "Failed to save store settings");
    } finally {
      setSaving(false);
    }
  };
  ...
  //     showError(err.response?.data?.message || "Failed to update profile image");
  //   } finally {
  //     setSaving(false);
  ...
  const saveNotifications = async () => {
    try {
      ...
      showError(err.response?.data?.message || "Failed to save notifications");
    } finally {
      setSaving(false);
    }
  };
            ...
            <div className="settings-card">
              ...
              <div className="profile-section">
                <div className="large-avatar">
                  {profileImage ? (
                    <img src={profileImage} alt={name || "Admin"} />
                  ) : (
